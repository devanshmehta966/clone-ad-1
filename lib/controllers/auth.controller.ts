...
import { SignUpSchema, SignInSchema, UpdateProfileSchema, NotificationPreferencesSchema } from '../validation/auth.schemas'
...
export class AuthController extends BaseController {
  ...
  async updateProfile(request: NextRequest, userId: string) {
    return this.handleRequest(async () => {
      const data = await this.validateBody(request, UpdateProfileSchema)
      const result = await this.authService.updateUserProfile(userId, data)
      return result
    })
  }

  async updateNotificationPreferences(request: NextRequest, userId: string) {
    return this.handleRequest(async () => {
      const data = await this.validateBody(request, NotificationPreferencesSchema)
      const result = await this.authService.updateNotificationPreferences(userId, data)
      return result
    })
  }
}