...
export class ReportsService {
  ...
  async generateReport(reportId: string, userId: string) {
    // Get raw report data from database (not serialized)
    const report = await prisma.report.findFirst({
      where: {
        id: reportId,
        userId
    ...
    return {
      ...updatedReport,
      startDate: updatedReport.startDate.toISOString(),
      endDate: updatedReport.endDate.toISOString(),
      createdAt: updatedReport.createdAt.toISOString(),
      emailSentAt: updatedReport.emailSentAt?.toISOString() || null
    }
  }
